...
class PreprocessReport:
    ...
    def __str__(self) -> str:
        ...
        if self.source_report:
            lines.append("=== 原始数据生产报告 ===")
            for k, v in self.source_report.items():
                lines.append(f"  {k:25s}: {v}")
        else:
            lines.append("=== 原始数据生产报告缺失 ===")
        lines.append("=== 预处理报告 ===")
...
def preprocess_directory(cfg: DictConfig) -> PreprocessReport:
    ...
    source_report_path = os.path.join(pc.input_dir, "creator_report.json")
    if os.path.isfile(source_report_path):
        with open(source_report_path, "r") as f:
            report.source_report = json.load(f)
    ...
    for fname in tqdm(episode_files, desc="Preprocessing episodes", unit="ep"):
        ...
        if report.dt == 0.0 and episode.dt > 0.0:
            report.dt = episode.dt * sample_interval
        samples = preprocess_episode(episode, history_len, future_len, sample_interval)
