...
class Conditional2DUNet(nn.Module, ModelMixin):
    def __init__(self, in_dim, in_ch, out_ch,
        ...
        self.input_dims = (in_ch, in_dim, in_dim)  # ModelMixin.rand_input -> (B, C, H, W)
        self.temb_ch = self.ch * embed_ch_mult

        # Embeddings
        # sigma embedding shape: (B, temb_ch)
        ...
        # Downsampling
        curr_res = in_dim                                        # spatial resolution H=W
        in_ch_dim = [ch * m for m in (1,)+ch_mult]              # channel plan per level
        ...
        for i, (block_in, block_out) in enumerate(pairwise(in_ch_dim)):
            ...
            for _ in range(self.num_res_blocks):
                # each block: (B, block_in,  curr_res, curr_res) -> (B, block_out, curr_res, curr_res)
                block = [make_block(block_in,block_out)]
                if curr_res in attn_resolutions:
                    # attn keeps shape: (B, block_out, curr_res, curr_res) -> same
                    block.append(AttnBlock(block_out))
                down.blocks.append(CondSequential(*block))
                block_in = block_out
            if i < self.num_resolutions - 1: # Not last iter
                # downsample: (B, block_in, curr_res, curr_res) -> (B, block_in, curr_res/2, curr_res/2)
        ...
        for i_level, (block_out, next_skip_in) in enumerate(pairwise(reversed(in_ch_dim))):
            ...
            for i_block in range(self.num_res_blocks+1):
                ...
                # concat before block:
                #   h        : (B, block_in, curr_res, curr_res)
                #   skip hs  : (B, skip_in,  curr_res, curr_res)
                #   cat      : (B, block_in + skip_in, curr_res, curr_res)
                # block output: (B, block_out, curr_res, curr_res)
            ...
            if i_level < self.num_resolutions - 1: # Not last iter
                # upsample: (B, block_in, curr_res, curr_res) -> (B, block_in, curr_res*2, curr_res*2)
    ...
    def forward(self, x, sigma, cond=None):
        # x: (B, in_ch, in_dim, in_dim)
        # sigma: scalar () or broadcastable batch form (e.g. (B,1,1,1))
        # cond: (B, ...) when conditional
        assert x.shape[2] == x.shape[3] == self.in_dim

        # Embeddings
        emb = self.sig_embed(x.shape[0], sigma.squeeze())  # (B, temb_ch)
        if self.cond_embed is not None:
            assert cond is not None and x.shape[0] == cond.shape[0], \
                'Conditioning must have same batches as x!'
            emb += self.cond_embed(cond)  # (B, temb_ch)

        # downsampling
        hs = [self.conv_in(x)]  # first feature map: (B, ch, in_dim, in_dim)
        for down in self.downs:
            for block in down.blocks:
                h = block(hs[-1], emb)  # keep curr_res, update channels per level
                hs.append(h)
            if hasattr(down, 'downsample'):
                hs.append(down.downsample(hs[-1]))  # halve H,W

        # middle
        h = self.mid(hs[-1], emb)  # bottleneck feature: (B, C_mid, H_mid, W_mid)

        # upsampling
        for up in self.ups:
            for block in up.blocks:
                skip = hs.pop()  # same spatial size as current h
                h = block(torch.cat([h, skip], dim=1), emb)
            if hasattr(up, 'upsample'):
                h = up.upsample(h)  # double H,W

        # out
        return self.out_layer(h)  # (B, out_ch, in_dim, in_dim)
